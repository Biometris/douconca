...
#' which the CWMs are calculated, and the environmental data
#' and weights for species and sites (the abundance totals for species and sites).
...
#' \code{formulaTraits} and a list of data with elements \code{dataEnv} and \code{dataTraits}.
...
checkCWM2dc_CA <- function(object, dataEnv, dataTraits, formulaTraits){
  ...
  if ("dataTraits" %in% names(object)) {
    if(is.null(dataTraits)) object[["data"]]$dataTraits <- object$dataTraits;
    object$dataTraits<-NULL
    }
  if ("dataEnv" %in% names(object)) {
    if(is.null(dataEnv)) object[["data"]]$dataEnv <- object$dataEnv;
    object$dataEnv<-NULL
    }
  ...
  # check weights
  if (is.null(object$weights)|| is.null(object$weights$columns) || is.null(object$weights$rows)) {
