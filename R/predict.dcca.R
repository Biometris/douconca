...
#' are included).
#'
#' For \code{type = "response"} and non-null \code{newdata}, the species weights 
#' of  the training are used; the site weights are taken equal. Many of the 
...
predict.dcca <- function(object,
  ...
  if (type == "response") {
    if (is.null(weights)) {
      weights = list(species = NULL, sites = NULL)
    }
    if (is.null(weights[[1]])) {
      weights$species <- rep(1 / nrow(newdata1[["traits"]]), 
                             nrow(newdata1[["traits"]]))
    }
    if (is.null(weights[[2]])) {
      weights$sites <- rep(1 / nrow(newdata1[["env"]]), nrow(newdata1[["env"]]))
    }
    if (!length(weights[[1]]) == nrow(newdata1[["traits"]])) {
      ...
      weights[[1]] <- rep(1 / nrow(newdata1[["traits"]]), 
                          nrow(newdata1[["traits"]]))
    }
    if (!length(weights[[2]]) == nrow(newdata1[["env"]])){
      ...
      weights[[2]] <- rep(1 / nrow(newdata1[["env"]]), nrow(newdata1[["env"]]))
    }
  }
  ret <- switch(type,
