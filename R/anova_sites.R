...
#
  ...
  if (is.null(object$CWMs_orthonormal_traits)) {
    warning(" Site level anova requires abundance data or community weighted means (CWMs")
...
# start of new dc-ca ------------------------------------------------------
  ...
  out1 <- object[c("CCAonTraits", "formulaTraits","data","weights","call","Nobs","CWMs_orthonormal_traits")]

  n <- out1$Nobs
  if (is.null(out1$CWMs_orthonormal_traits)){
    CWMs_orthonormal_traits <- vegan::scores(object$CCAonTraits, display= "species",
                                             scaling = "species", choices = 1:Rank_mod(object$CCAonTraits))
  } else
...
sWn <- sqrt(object$weights$rows)
Yw <-  CWMs_orthonormal_traits*sWn
msqr <- msdvif(object$formulaEnv, object$data$dataEnv, object$weights$rows, XZ = FALSE)
