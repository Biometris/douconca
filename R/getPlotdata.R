#'  Utility function: extracting data from a \code{\link{dc_CA}} object for plotting
#'  a single axis by your own code or \code{\link{plot_dcCA}}.
#' @description
#'  \code{getPlotdata} extracts data from a \code{\link{dc_CA}} object for plotting the
...
#' If set to \code{NA}, no additional coloring and lines are displayed in \code{\link{plot_dcCA}}.
#' The parameter sets the \code{groups} variable in the \code{CWM_SNC} data frame of the return value/in the plot
...
#' names derived from the result of \code{\link{scores.dcca}} with \code{tidy=TRUE}.
...
getPlotdata <- function(object, axis=1, envfactor=NULL, traitfactor=NULL,newnames = NULL,
  ...
  if (is.null(envfactor)){
    ...
    if (ff$formula_Z == ~1){envfactor <- ff$focal_factor[1]; envINcondition <- FALSE }else{
      envfactor <- ff$Condi_factor[1]; envINcondition <- TRUE
    }
  }

  if (is.null(traitfactor)){
    ff<- get_Z_X_XZ_formula(object$formulaTraits, object$data$dataTraits)
    if (ff$formula_Z == ~1){traitfactor <- ff$focal_factor[1]; traitINcondition <- FALSE }else{
      traitfactor <- ff$Condi_factor[1]; traitINcondition <- TRUE
    }
  }
