...
anova.wrda <- function(object, permutations =999, by = NULL){
# object dcca object;
#
...
#Xw <- Xw[,-seq_len(ncol(Zw)), drop = FALSE]
...
# what the env. variables explain of the trait-structured variation
  ...
  axsig_dcCA_sites <- data.frame(df =df, `Variance`= ss,R2 = fraqExplained, `F`  = F0,`Pr(>F)` = p_val_axes1)
  names(axsig_dcCA_sites)[5]<- "Pr(>F)"

  object1 <- paste("Model:", c(object$call),"\n")

  head <- paste0("Permutation test for weighted reduncancy analysis\n",
                 object1,
