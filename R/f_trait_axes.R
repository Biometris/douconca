...
f_trait_axes <- function(out, which_cor = "in model"){
  ...
  if (is.null(out$data$Y) && is.null(out$SNC)) warning("SNC analysis is not available (e.g. no unconstrained species scores).")
  if (is.null(out$data$Y)){
    ...
    out$CCAonTraits$CCA$QR <- step2$CCA$QR
      ...
      lc_traits_scores = lc_trait_scores), correlation = NULL, c_traits_normed= out$c_traits_normed0, b_se= NULL, R2_traits = NULL))
    }
    #c_traits_normed/mod$c_traits_normed[,4:8]
    ...
    if (!is.null(out$data$Y)){
      ...
      } else if ("dcca" %in% class(out)){
       lc_scores <- out$RDAonEnv$site_axes$site_scores$lc_env_scores
      } else stop("first argument must be of class dcca or dccav")
      SNC <-  (t(as.matrix(out$data$Y)) %*% lc_scores) / (out$weights$columns * out$Nobs)
      #QR<- get_QR(out$CCAonTraits)
    } else {warning("something wrong in f_trait_axes") ;return(NULL)}
  ...
  if (!is.null(out$SNCs_orthonormal_env) && is.null(out$data$Y)){
   if(!is.null(out$c_traits_normed0)){
      ...
      c_traits_normed[, 3 + length(rseq) + rseq ] <-   c_traits_normed[, 3 + length(rseq) + rseq, drop = FALSE ] %*% flip
      res$y <- res$y %*% flip
      res$fitted <- res$fitted %*% flip
      res$b_se[, rseq] <-  as.matrix(res$b_se[, rseq, drop = FALSE])%*% flip
      SNC <- SNC%*%flip
   } else { warning(paste("The orientation of the trait and environmental axes cannot be aligned,",
        "as information is missing how the orthonormized traits must be backtransformed to the original traits."))}
     }
  ...
  if (which_cor[1] == "in model") {
    ...
    traits0 <- stats::model.matrix(gg$formula_X0, data = out$data$dataTraits)
  } else {whichc = which_cor
    traits0 <-  stats::model.matrix(~.-1, data = out$data$dataTraits[, whichc, drop= FALSE])
  }
  ...
  out2 <- list(species_scores = list(species_scores_unconstrained = res$y,
                                     lc_traits_scores = res$fitted), correlation = Cor_Trait_SNC, c_traits_normed= c_traits_normed, b_se= res$b_se, R2_traits = res$R2, step2 = step2)
  return(out2)
}
