...
backscale_data <- function(pred_scaled,
                           msd) {
  # pred_scaled  should  be matrix and msd should be a data frame with a 
  # column mean and rownames
...
check_newdata <- function(object, 
  ...
  newdata1 <- scale_data(dat0, mean_sd = avgSDS[, 1:2, drop = FALSE])
  return(newdata1)
}
...
predict_fc <- function(object, 
                       rank) {
  # fourth-corner coefficients of transformed response on env and trait 
  # predictors value is env by trait
