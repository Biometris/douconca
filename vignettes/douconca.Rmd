...
[@peng2021double;@gobbi2022hay;@pinho2021functional].
...
of which is the final P-value. The permutations are
specified via `permute` package [@permute2022] like they are
in the `vegan` package, which allow for analysis of
...
may contain factors, quantitative variance and transformations thereof, and interactions,
...
The double constrained version of principal components analyis also exists and is
...
such as CWM-regression and fourth-corner correlation analysis, that have proven to be 
...
The entry `traits_explain` is `r round(mod_e$inertia["traits_explain","weighted variance" ],3)`, which is the variance in the abundance data that is explained by the traits. 
...
traits and environmental variables jointly. Its value in the second model  
...
dc-CA axes are significant:
```{r}
anova(mod_e, by = "axis")$max
```

The fourth-corner correlation of the best linear combination of the
ecological traits with the best linear combinination of the environmental variables is 
```{r}
...
We first extract the regression coeffients from the dc-CA model using the `scores` function.
The second line calculates the regression coeffients by
dividing the standardize regression coeffients from `dc_CA` by the standard deviation
