...
Prediction of response and lc scores from environment data using
[`cca0`](cca0.md) and [`wrda`](wrda.md) models.
...
- object:
  ...
  return value of [`cca0`](cca0.md) or [`wrda`](wrda.md).
...
- type:
  ...
  type of prediction, `c( "response", "lc")` for response (expected
  abundance) and constrained scores for sites.
...
In a [`cca0`](cca0.md) model with `type = "response"`, many of the
predicted values may be negative, indicating expected absences (0) or
small expected response values.
