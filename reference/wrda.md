...
- response:
  ...
  Rownames of `response`, if any, are carried through. Can be `NULL` if
  `cca_object` is supplied or if the response is `formula` is two-sided.
...
- cca_object:
  ...
  a vegan-type cca-object of *transposed* `response`, from which centred
  abundance values and row and column weights can be obtained.
...
All scores in the `wrda` object are in scaling `"sites"` (1): the
...
Compared to
[`rda`](https://vegandevs.github.io/vegan/reference/cca.html), `wrda`
does not have residual axes, *i.e.* no SVD or PCA of the residuals is
performed.
...
[`scores.wrda`](scores.wrda.md), [`anova.wrda`](anova.wrda.md),
[`print.wrda`](print.wrda.md)
...
#> wRDA      6   36.495 0.62581 3.3449  0.008 **
#> Residual 12   21.822                         
#> ---
